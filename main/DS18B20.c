...
static esp_err_t DS18B20_start_RESET_sequence(void){
    ...
    int64_t elapsed_time = esp_timer_get_time() - sensor_PRESCENCE_start;
    esp_rom_delay_us(DS18B20_PRESENCE_WAIT - elapsed_time);
...
esp_err_t read_DS18B20(void){
    ...
    if (DS18B20_start_RESET_sequence() != ESP_OK) {
    ...
    while(esp_timer_get_time() - tconv_START_time < LONG_TIMEOUT){
        esp_rom_delay_us(DS18B20_CONVERSION_POLL_INTERVAL);
        if(DS18B20_read_bit()){
            break;
        }
    }
